...
def ensure_storage_exists():
    ...
    try:
        ...
        if not any(b.id == BUCKET_ID for b in buckets):
            ...
            print(f"ℹ️ Storage bucket '{BUCKET_ID}' is ready.")
...
async def upload_ecg(
    ...
    return {"message": "File secured in cloud storage", "url": file_url}
...
def predict_from_file(file_id: int, db: Session = Depends(get_db), email: str = Depends(get_current_user)):
    ...
    try:
        ...
        history = History(
            ...
            probability=prediction["average_probability"], 
            user_id=user.id
        )
        db.add(history)
        db.commit()
...
@app.get("/")
def root():
    return {"status": "Operational", "engine": "Cloud-Native Neural Core"}
